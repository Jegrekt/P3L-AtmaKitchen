...
import { getLaporanPemasukanDanPengeluaran } from "../../../../api/ApiLaporan";
import Table from "../../../../ui/Table";
import ButtonBack from "../../../../ui/ButtonBack";
import { setDate } from "../../../../utils/setDate";
import { monthNames } from "../../../../utils/Constants";
import { rupiah } from "../../../../utils/rupiah";
import LPDPRow from "./LPDPRow";
function LaporanPemasukanDanPengeluaran() {
  ...
  useEffect(() => {
    setLoading(true);
    getLaporanPemasukanDanPengeluaran(month, year)
      .then((res) => {
        setData(res);
        console.log(res);
        setLoading(false);
        ...
        <Card className="max-w-4xl mx-auto mt-10 print:shadow-none print:bg-transparent print:mt-0 LPP">
          <CardBody>
            ...
            <Table columns="grid-cols-[2fr,2fr,2fr]">
              ...
              <Table.Body
                data={data}
                render={(item) => <LPDPRow key={item.nama} item={item} />}
              />
            </Table>
            <div className="flex justify-between mt-4">
              <p className="text-right font-medium">Total Pengeluaran : </p>
              <p className="font-numeric text-red-400 text-lg">
                {rupiah(data.reduce((acc, item) => acc + item.pengeluaran, 0))}
              </p>
            </div>
            <div className="flex justify-between mt-4">
              <p className="text-right font-medium">Total Pemasukan : </p>
              <p className="font-numeric text-green-400 text-lg">
                {rupiah(data.reduce((acc, item) => acc + item.pemasukan, 0))}
              </p>
            </div>
          </CardBody>
        </Card>
