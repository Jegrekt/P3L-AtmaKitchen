...
import { FaEye, FaLocationDot, FaPrint } from "react-icons/fa6";
import { setDate } from "../../../utils/setDate";
import { rupiah } from "../../../utils/rupiah";
import { IoClose } from "react-icons/io5";
import DetailHisotoryRow from "./DetailHisotoryRow";
import { useNavigate } from "react-router-dom";

function ProfilCustomerRow({ item }) {
      ...
      <Table.Row>
        ...
        <Menu>
          ...
          <MenuList>
            ...
            <MenuItem onClick={() => navigate(`/view-pemesanan/${item.id}`)}>
              <div className="flex gap-4">
                <FaPrint />
        ...
        <DialogBody className="pt-4">
          <div className="flex justify-between items-start text-black gap-6">
            <div className="flex-1">
              ...
              <p className="flex justify-between">
                <span>Harga</span>
                <span>{rupiah(dataTrx.grand_total)}</span>
              </p>
              <p className="flex justify-between">
                <span>Tip</span>
            ...
            <div className="flex-1">
              ...
              {dataTrx?.jenis_pengiriman == "Delivery" && (
                <>
                  {dataTrx?.id_alamat != null ? (
                    <>
                      <p className="flex justify-between">
                        <span>Biaya Pengiriman</span>
                        <span>
                          {!dataTrx.ongkos_kirim
                            ? "———"
                            : rupiah(dataTrx.ongkos_kirim)}
                        </span>
                      </p>
                      <p>
                        <span>Alamat</span>
                        <span>{dataTrx?.alamat?.alamat}</span>
                      </p>
                      <p className="flex justify-between">
                        <span>Jarak</span>
                    ...
                    <p className="flex justify-center items-center text-red-500">
                      <span>[!] Alamat belum dikonfirmasi [!]</span>
                    </p>
                  )}
                </>
              )}
...
export default ProfilCustomerRow;
